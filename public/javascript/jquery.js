...
$(".add").click(function(){
    ...
    $.ajax({
        url:"/trangchu/giohang/addcart",
        type:"POST",
        dataType:"html",
        data:{ id: id, soluong: num,
        },
        success: function(){ 
...
$(".btn-rank").click(function(){
	star = $(".valstar").val();
	id = $(".btn-rank").attr("data-id")
	if (star > 0)
	{
		$(".btn-rank").attr('disabled', 'disabled');
            $.ajax({
                url:"/trangchu/sanpham/danhgia",
                type:"POST",
                dataType:"html",
                ...
                success: function(data){ 
                    if (data == 1){
                        ...
                        alert("Bạn Đã Dánh Giá Sản Phẩm Này")
                    }
                    else{
						
                        $(".rank").html(data);
                        $(".rang").removeClass("rang-1");
                        $(".rang").removeClass("rang-2");
                        $(".rang").removeClass("rang-3");
                        $(".rang").removeClass("rang-4");
                        $(".rang").removeClass("rang-5");
...
$(".btn-comment.sanpham").click(function(){
	noidung = $(this).parent().children(".bl-comment").val().replace(/\r?\n/g, "<br>");
	bl = $(this).attr("data-bl")
